...
| `.dialog-opt` | the shared dialog's OPT-OUT: one checkbox that changes whether the dialog RETURNS, not what it answers | built by `_dlgOpen` from a `check` object, read with `dlgChecked()` after it resolves. **Never gates OK**, and is read on every route out including Escape and the backdrop — unticked is the ordinary answer, which is what separates it from `.dialog-choices` above. Sits with the buttons, muted and `--text-sm`, so it cannot compete with them |
